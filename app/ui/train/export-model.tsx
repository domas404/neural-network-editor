...
const ExportElement = ({name, icon, fileFormat, handleClick}: ExportElementProps) => {
    return (
        <div className="bg-white dark:bg-slate-800">
            <div
                className="rounded-lg hover:bg-slate-100 hover:cursor-pointer p-2 dark:hover:bg-slate-700
...
export default function ExportModel() {
    ...
    const saveModelArchitecture = () => {
        const model = localStorage.getItem("tensorflowjs_models/my-model/model_topology");
        console.log("Model", model);
    }

    const saveModelWeights = () => {
        const model = localStorage.getItem("tensorflowjs_models/my-model/weight_data");
        console.log("Model", model);
    }
            ...
            <div className="py-5 w-full">
                ...
                <div className="text-md px-4">
                    <div className="flex flex-col bg-gray-200 mt-2 dark:bg-slate-700">
                        <ExportElement name="Model architecture" icon="code" fileFormat="JSON" handleClick={saveModelArchitecture} />
                        <ExportElement name="Model weights" icon="code" fileFormat="BIN" handleClick={saveModelWeights} />
