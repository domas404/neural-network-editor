...
function CustomConvolutionKernel({ data }: any) {
    return (
        <>
            <Handle type="target" id="a" position={Position.Left} isConnectable={false} className="hidden" />
            <div className="w-10 h-10 rounded-full bg-white
                dark:bg-slate-800 relative">
                    <div className='absolute w-10 h-10 border-4 border-lightblue-700 dark:border-blue-300 opacity-10'></div>
                    <div className='absolute m-1 w-8 h-8 border-4 border-lightblue-700 dark:border-blue-300 shadow-[0_0_4px_4px_rgba(147,197,253,0.3)]'>
                        <div className={`w-6 h-6 border-4 border-transparent ${activationColors[data.activation]}`}></div>
                    </div>
...
export default memo(CustomConvolutionKernel);